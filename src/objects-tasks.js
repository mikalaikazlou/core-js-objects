...
function mergeObjects(objects) {
  ...
  for (let index = 0; index < objects.length; ) {
    const subObj = Object.entries(objects[index]);
    subObj.forEach((val) => {
      const [key, value] = val;
      if (mergeResult[key]) {
...
function makeWord(lettersObject) {
  const maxIndex = [...Object.keys(lettersObject).join().replaceAll(',', '')];
  const array = Array(maxIndex).fill('');
  Object.entries(lettersObject).reduce((acc, [letter, positions]) => {
...
function Rectangle(width, height) {
  const rctngl = {};
  rctngl.width = width;
  rctngl.height = height;
  rctngl.getArea = () => {
    return rctngl.width * rctngl.height;
  };
  return rctngl;
}
